...
class Connection {
  ...
  async connect(sessionParams = {}) {
    ...
    const connected = conn.once("databaseConnected", this.networkTimeout);
    conn.send({
    ...
    const query = async (statement, ...replacements) => {
      ...
      try {
        statementResponse = await conn.once("statementId", this.networkTimeout);
      } catch (e) {
        done();
      ...
      const enqueue = async () => {
        ...
        try {
          ...
          if (ipResponse.reconnect) {
            ...
            const databaseResponse = await conn.once("databaseConnected", this.networkTimeout);
            queryObj.varchar_encoding = databaseResponse.varcharEncoding;
            conn.send({"reconstructStatement": queryObj.statement_id});
            try {
              await conn.once("statementReconstructed", this.networkTimeout);
            } catch (e) {
        ...
        const exec = async () => {
          ...
          try {
            ...
            await conn.once("queryType", this.networkTimeout);
        
            conn.send({"queryTypeOut": "queryTypeOut"});
            const queryTypeReponse = await conn.once("queryTypeNamed", this.networkTimeout);
            ...
            return {
              ...
              close: async ()=> {
                conn.send({closeStatement: "closeStatement"});
                return conn.once("statementClosed", this.networkTimeout).finally(done)
              }
            };
          } catch (err) {
            conn.send({closeStatement: "closeStatement"});
            try {
              await conn.once("statementClosed", this.networkTimeout);
            } catch(e) {}
            throw err;
          }
        }
