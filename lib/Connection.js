    ...
    executeQuery(sql, callback, setCommand, hostArg, portArg) {
        ...
        client.on('data', function (data) {
            ...
            if (setCommand && !self.setCommandRun) {
                switch (keys[0]) {
                    ...
                    case 'databaseConnected':
                        ...
                        self.varchar_encoding = response.varcharEncoding || 'ascii'
                        self.handleMessage.set_varchar_encoding(self.varchar_encoding)
            ...
            switch (keys[0]) {
                case 'connectionId':
                case 'databaseConnected':
                    self.varchar_encoding = response.varcharEncoding && response.varcharEncoding.includes('874') ? 'cp874' : 'ascii'
                    self.handleMessage.set_varchar_encoding(self.varchar_encoding)
