...
module.exports = function ParseSqreamData() {
  ...
  function set_varchar_encoding(encoding) {
    this.varchar_encoding = encoding || 'ascii';
  }

  const fetchBufferByColType = {
   currentVarcharPosition: 0,
    ftLong: function (dataBuffer, fieldSize, i) {
       var numBuf = dataBuffer.slice(i * fieldSize, (i * fieldSize)+fieldSize);
       const value = sqreamCppBuffer.bufferToLong(numBuf);
       return value;
    },
    ftVarchar: function (dataBuffer, fieldSize, i) {
      const varCharPosition = i * fieldSize;
      return iconv.decode(dataBuffer.slice(varCharPosition, varCharPosition + fieldSize), this.varchar_encoding);
    },
