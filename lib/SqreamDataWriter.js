...
function convertDate(year, month, day, hours, minutes, seconds, millis) {
  ...
  const a = (365 * year + Math.floor(year / 4) - Math.floor(year / 100) + Math.floor(year / 400) + Math.floor((month * 306 + 5) / 10) + (day - 1));
  if (hours = undefined) return [a];

  const b = (hours | 0) * 3600000 + (minutes | 0) * 60000 + (seconds || 0) * 1000 + (millis | 0);
  return [a, b];
}
...
const bitconverter = {
  ...
  ftDate: (buf, offset, value) => {
    ...
    buf(4).writeInt32LE(convertDate(date.getFullYear(), date.getMonth(), date.getDate())[0], offset);
    return 4;
  },
  ftDateTime: (buf, offset, value) => {
    const date = new Date(value);
    const nums = convertDate(date.getFullYear(), date.getMonth(), date.getDate(), date.getHours(), date.getMinutes(), date.getSeconds(), date.getMilliseconds());
    buf(8).writeBigInt64LE((BigInt(nums[0]) << BigInt(32)) + BigInt(nums[1]), offset);
    return 8;
  },
...
module.exports = {
  ...
  createInsertBuffer: function (queryTypeIn, varcharEncoding, maxBytes) {
    ...
    const perRow = queryTypeIn.reduce((sum, meta) => (meta.type[1] ? meta.type[1] : 100) + sum + meta.nullable, 0);
    const colSizes = queryTypeIn.map((meta) => Math.ceil((meta.type[1] ? meta.type[1] : 100) / perRow * maxBytes));
