...
function parseTime(timeString) {
  ...
  const hours = (Number(parts[0]) | 0) * 3600000;
  const minutes = (Number(parts[1]) | 0) * 60000;
  const seconds = (Number(parts[2])  * 1000) | 0;
...
const bitconverter = {
  ...
  ftDate: (buf, offset, value) => {
    buf(4).writeInt32LE(parseDate(value), offset);
    return 4;
  },
  ftDateTime: (buf, offset, value) => {
    const parts = ( value + "").split(" ");
    buf(8).writeBigInt64LE((BigInt(parseDate(parts[0])) << BigInt(32)) + BigInt(parseTime(parts[1])), offset);
    return 8;
  },
