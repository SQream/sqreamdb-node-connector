...
module.exports = function sqConnect(host, port = 5000, is_ssl = false, debug = false, connectTimeout = 3000) {
  if (typeof host != 'string' || typeof port != 'number') return Promise.reject(new Error(`Bad host or port: ${host}:${port}`));
  const socket = is_ssl ? tls : new net.Socket();
  let client;
  ...
  return new Promise((connRes, connRej) => {
    ...
    function close(reason = "Connection closed unexpectedly") {
      if (!alive) return;
      alive = false;
    ...
    client = socket.connect({port: port, host: host, rejectUnauthorized: false})
    ...
    const connectTimer = setTimeout(() => connRej(new Error("Host unreachable or unresponsive.")), connectTimeout);
    client.on(is_ssl ? 'secureConnect' : 'connect', () => {
      alive = true;
      clearTimeout(connectTimer);
