...
module.exports = function sqConnect(host, port = 5000, is_ssl = false, debug = false) {
  ...
  const client = is_ssl ? new tls.TLSSocket(new net.Socket()) : new net.Socket();
  ...
  return new Promise((connRes, connRej) => {
    ...
    client.on('connect', () => {
      ...
      connRes({subscribe, once, send, clear, onError, close, interupt, isAlive, firstBuffer, setMaxRows});
    });
